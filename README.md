...
将东西（一个 `RewriteRule` 文件夹）丢到 `usr/plugins` 下即可。
...
|   C|按照条件查找文章。|这个有点复杂，注意看下面的[应用举例1](#sample1)

##应用举例

<h3 id='sample1'>解决永久链接的混乱情况</h3>
####一个故事
John 用 Typecho 搭建了一个 Blog，写了一些文章，但是由于一直没有设置永久链接，因此他的文章地址一直是 `/archives/123/` 这样的。
